...
- Memory vector search is now reliable across restarts. The local embedding was a stateful TF-IDF with a mutable slot index and a non-persistent in-memory vocabulary, so a stored entry's embedding and a later query's embedding were computed in different projections — after a restart the vector score was effectively noise. It's now a stateless hashing-trick embedding (stable token→slot hash, normalized term frequency): the same text always yields the same vector regardless of history, instance, or restart, and embedding a query no longer mutates any state.
- The memory index no longer carries a dead `embeddings` cache that was loaded and re-serialized but never populated at runtime; loaded entries are also validated so a hand-edited or partially-written index can't insert a bogus record.
- The cron service is now started once at gateway bootstrap, bound to the global agent. It was being (re)initialized inside per-user `createAgent()`, where the process-wide singleton silently discarded each user's executor and stayed bound to whichever runtime happened to be created first — a fragile, ordering-dependent setup. The service is also stopped on shutdown.
- Per-user agents now get an isolated working directory (`.../workspace/users/<id>/`) for their bash/filesystem tools, instead of all users sharing `process.cwd()`. The bash tool defaults its working directory and the filesystem tools resolve relative paths against the sandbox, so scoping doesn't lock the tools out.
- A hung cron job no longer wedges the whole scheduler. Job execution is now bounded by a timeout (the job's `timeoutSeconds`, or a 10-minute default), so a job that never returns is marked errored and the scheduler keeps running instead of getting stuck with `running` set forever.
- Overdue one-time (`at`) jobs whose window passed while the process was down now emit a `missed` event and are disabled, instead of silently vanishing.
- A manual `run` on a job that is already running now returns `skipped` instead of double-executing it.
- Creating a cron job with a name that already exists is rejected instead of producing an ambiguous duplicate.
...
- `web_fetch` is now guarded against SSRF. It previously fetched any model-supplied URL with no restriction and followed redirects, so a prompt-injected agent could reach cloud metadata (`169.254.169.254`, `metadata.google.internal`) or internal services and return the contents. It now allows only `http`/`https`, rejects private/reserved/link-local/metadata addresses (both literal hosts and DNS-resolved, defeating DNS rebinding), and follows redirects manually — re-validating every hop so a public URL can't redirect into the internal network. Self-hosters who need internal fetches can set `VEX_WEB_FETCH_ALLOW_PRIVATE=1`.
- `web_fetch` now streams the response with a hard byte cap and a request timeout, instead of buffering the whole body into memory before truncating (a memory-exhaustion vector) and hanging indefinitely on a slow server.
