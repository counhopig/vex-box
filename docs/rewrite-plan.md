...
| `providers/{ProviderMetadata,ModelResolver,ProviderInterface,fetch-compat}.ts` | ✅ 完成 | TDD, 15 tests, tsc clean。ProviderMetadata: 17 entries table. ModelResolver: 函数式 API (initModelResolver/resolveModel/getApiKeyForProvider/isProviderAvailable), 3 步解析路径 (China table → pi-ai getModel → dynamic fallback). ProviderInterface: LLMProvider/ChatMessage/ChatResponse 类型. fetch-compat: 非 ASCII 响应头补丁 (从 cli/fetch-patch.ts 迁出). |

下一步：`agent/AgentRuntime.ts` + `agent/messages.ts` —— 将当前 Agent.ts 的 ChatFn DI 替换为真实的 pi-coding-agent 集成（session 管理、streaming、tool loop）。
