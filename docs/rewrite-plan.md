...
| `hooks/{types,EventBus,index}.ts` | ✅ 完成 | TDD, 18 new tests (416 total), tsc clean。types.ts: HookEvent 判别联合 (message_received/sending/sent, agent_start/end, tool_start/end, error) + HookHandler 类型。EventBus.ts: **class-based** (archive 是 module-level Map，违反 principle 5；新设计用 class 但保留 defaultBus 实例用于 app-wide hooks)，subscribe/unsubscribe/subscribeMany/emit/emitSync/clear/handlerCount，emit 中隔离 handler 错误。index.ts: barrel + 便利 emit* 函数（emitMessageReceived/.../emitError），委托给 defaultBus.emitSync()。 |
