...
- **Persona profile follow-up**：`PersonaStorage` 的画像此前只存在于单个 Agent 内存中，且 `observeResponse()` 从未提取事实，与架构文档承诺不符。现已先以保守规则支持用户明确声明姓名，原子持久化到 per-user memory 目录并在后续 Persona prompt 的【用户画像】段注入；不从模型回复或含糊表达推断身份。
- **Persona observer wiring follow-up**：线上 QA 继续发现 `Persona.observeResponse()` 虽已实现但 `Agent.processMessage()` 从未调用，导致结构化画像文件仍不会生成。现已在 LLM 成功回复后、Pipeline observers 之前接入该调用，并以 Agent 全流程测试断言画像与近期对话确实更新。
