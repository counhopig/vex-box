...
Vex 是一个轻量级的 AI 助手框架，专注于国产生态。基于 [pi-coding-agent](https://github.com/nicemicro/pi-coding-agent) 构建 Agent 运行时（内置会话管理、上下文压缩、工具执行），使用 [pi-ai](https://github.com/nicemicro/pi-ai) 作为统一的多模型调用层（支持 25+ 提供商），原生支持 Function Calling，并支持 QQ、飞书、钉钉、企业微信等通讯平台。
...
Vex 的架构设计参考了 [Moltbot](https://github.com/moltbot/moltbot)，但专注于不同的使用场景：
...
> **Vex 用 3% 的代码量实现了核心功能**，专注简洁高效，易于理解和二次开发。
...
```json5
{
  ...
  skills: {
    ...
    userDir: "~/.vex/skills",     // 用户级技能目录
    workspaceDir: "./.vex/skills", // 工作区级技能目录
  ...
  memory: {
    ...
    storageDir: "~/.vex/memory"   // 存储目录（默认 ~/.vex/memory）
  }
}
```
...
Vex 支持从 [ClawdHub](https://clawhub.ai) 搜索和安装社区共享的技能。安装 `clawhub` CLI 后，Agent 会自动获得搜索和安装技能的能力。
...
> ClawdHub 安装的技能使用 moltbot 的 frontmatter 格式（`metadata.openclaw.requires`），Vex 会自动兼容解析。

## 记忆系统

记忆系统让 Agent 能够跨会话记住重要信息，如用户偏好、关键事实、任务上下文等。记忆默认启用，存储在 `~/.vex/memory/` 目录。
...
也可以通过 `vex onboard` 向导配置记忆系统（步骤 5/5）。
...
任务数据存储在 `~/.vex/cron/jobs.json`，支持持久化。

## 插件系统

插件系统允许扩展 Vex 的功能，支持自动发现和加载。
...
|--------|------|------|------|
| 1 | 内置 | `plugins/` | 项目自带插件 |
| 2 | 全局 | `~/.vex/plugins/` | 用户安装的全局插件 |
| 3 | 工作区 | `./.vex/plugins/` | 项目级插件 |
...
```bash
# 配置
vex onboard            # 配置向导（模型/平台/服务器/Agent/记忆系统）
vex check              # 检查配置
vex models             # 列出可用模型

# 启动服务
vex start              # 完整服务（含 QQ/飞书/钉钉）
vex start --web-only   # 仅 WebChat
vex start --port 8080  # 指定端口

# 服务管理
vex status             # 查看服务状态（进程数、CPU/内存、健康检查）
vex restart            # 重启服务（支持 --web-only 等选项）
...
如果你想了解 AI Agent 的工作原理，Vex 是一个很好的学习项目。相比动辄几十万行代码的大型框架，Vex 只有约 16,000 行代码，但实现了完整的 Agent 核心功能。
...
Vex 提供完整的 Docker 部署支持，包含 Dockerfile 和 Docker Compose 配置。
...
数据通过 Docker volume `vex-data` 持久化，包含：
