/**
 * JsonMemoryStore — JSON file-backed memory store.
 *
 * Ported from _archive/src/memory/store.ts. Safety behaviors preserved:
 *  - Atomic write: write to `<index>.tmp` then rename over `index.json`.
 *    rename(2) is atomic on POSIX, so a crash or an overlapping writer can
 *    never leave a truncated index.json that the next load would fail to
 *    parse. Proven by a test: with a read-only index file (0o444) an
 *    in-place write would EACCES, but temp-write + rename still succeeds.
 *  - Entries are validated on load: a hand-edited or partially-written index
 *    must not store entries under a bogus key (e.g. `undefined`).
 *  - `clear()` unlinks the index file instead of writing an empty one.
 */
...
/** Generate a unique entry id (16 hex chars, collision-resistant enough). */
...
/** Cosine similarity for two vectors — 0 on length mismatch or empty input. */
...
export class JsonMemoryStore implements MemoryStore {
  ...
  constructor(options: {
    ...
    this.directory = options.directory
      ? expandHomePath(options.directory)
      : path.join(os.homedir(), ".vex", "memory");
  ...
  private saveIndex(): void {
    ...
    try {
      ...
      // Atomic write: temp file + rename. rename is atomic on POSIX, so a
      // crash or an overlapping writer can never leave a truncated index.json
      // that the next loadIndex would fail to parse. Also proven by the
      // read-only-index test: rename only needs directory write permission.
