 ...
 *
 * Ported from _archive/src/channels/weixin/client.ts as WeChatClient.
 * Safety behavior preserved: every response body is passed through
 * `assertOkEnvelope` — the OC API signals failures (session timeout, invalid
 * context_token, ...) in the body with non-zero ret/errcode, so HTTP success
 * alone never means the call succeeded.
 */
...
export class WeChatClient {
  ...
  private buildBaseHeaders(tokenRequired = false): Record<string, string> {
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
      AuthorizationType: "ilink_bot_token",
      "X-WECHAT-UIN": WeChatClient.generateXWechatUin(),
    };
  ...
  /** Fetch messages (long polling). Calls ilink/bot/getupdates. */
