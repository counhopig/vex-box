...
import { COMMON_CSS, WEBCHAT_CSS, CONTROL_CSS } from "./styles.js";
import { WEBCHAT_CLIENT_JS, CONTROL_CLIENT_JS } from "./client.js";
...
function getEmbeddedHtml(config: SystemConfig): string {
  const assistantName = "Vex";
  const agent = (config.agent ?? {}) as Record<string, unknown>;
  const defaultModel = escapeHtml(typeof agent.defaultModel === "string" ? agent.defaultModel : "");
  const defaultProvider = escapeHtml(typeof agent.defaultProvider === "string" ? agent.defaultProvider : "");
...
/** Get Control UI page */
function getControlHtml(config: SystemConfig): string {
  const assistantName = "Vex";
  const agent = (config.agent ?? {}) as Record<string, unknown>;
  const defaultModel = escapeHtml(typeof agent.defaultModel === "string" ? agent.defaultModel : "");
  const defaultProvider = escapeHtml(typeof agent.defaultProvider === "string" ? agent.defaultProvider : "");
...
export interface StaticServerOptions {
  /** System config for the control-panel defaults (agent model/provider). */
  config: SystemConfig;
  /** Authentication — isEnabled gates /login + protected pages; getRequestUser
   *  resolves the session cookie. Replaces the archive's module-global
   *  getRequestUser/isWebAuthEnabled(config) helpers (principle #5). */
  auth: WebAuthStore;
}
...
export function handleStaticRequest(
  ...
  if (options.auth.isEnabled && pathname === "/login") {
    const currentUser = options.auth.getRequestUser(req);
    if (currentUser) {
  ...
  if (options.auth.isEnabled && protectedPage && !options.auth.getRequestUser(req)) {
  ...
  if (pathname === "/control" || pathname === "/control/") {
    ...
    if (options.auth.isEnabled) {
      const user = options.auth.getRequestUser(req);
