 ...
 *
 * Ported from archive with identical API behavior, timeouts, and cookie
 * handling. SSRF-safe: only fetches bilibili.com / b23.tv endpoints.
 */
...
export class BilibiliAdapter extends BasePlatformAdapter {
  ...
  match(url: string): boolean {
    ...
    try {
      ...
      if (isBilibiliHost(parsed.hostname) || parsed.hostname === "b23.tv") {
        return true;
      }
    } catch {
      // ignore invalid URL
  ...
  async resolveUrl(url: string): Promise<string | undefined> {
    ...
    if (current.protocol !== "https:" || current.hostname !== "b23.tv") {
      return undefined;
    }
    ...
    try {
      ...
      for (let redirects = 0; redirects < 5; redirects += 1) {
        ...
        const allowed = next.protocol === "https:"
          && (next.hostname === "b23.tv" || isBilibiliHost(next.hostname));
        if (!allowed) {
          logger.warn({ hostname: next.hostname }, "Blocked unsafe b23.tv redirect");
          return undefined;
        }
        current = next;
