/**
 * Built-in tools — Weather query (wttr.in + Caiyun).
 *
 * Ported from _archive/src/tools/builtin/weather.ts. Key changes:
 *   - Uses Tool type from ../types.js (not AgentTool from pi-agent-core).
 *   - Config passed via WeatherToolOptions instead of full VexConfig slice.
 *   - Cache, coordinate table, and query logic preserved verbatim.
 */
...
export interface WeatherToolOptions {
  /** Selected provider (default: wttr). */
  provider?: WeatherProvider;
  /** CaIyun API key. */
  caiyunApiKey?: string;
  /** CaIyun API version. */
  caiyunApiVersion?: "v2.6" | "v3";
  /** Default location (city name or coordinates). */
  defaultLocation?: string;
  /** wttr base URL (default: https://wttr.in). */
  wttrBaseUrl?: string;
...
async function queryWttr(params: {
  ...
  const url = new URL(
    `${params.baseUrl.replace(/\/+$/u, "")}/${encodeURIComponent(params.location)}`,
  );
  ...
  const nearestArea = Array.isArray(data.nearest_area)
    ? data.nearest_area[0]
    : undefined;
  ...
  const weather = Array.isArray(data.weather)
    ? data.weather.slice(0, 3)
    : [];
...
async function queryCaiyun(params: {
  ...
  const url = new URL(
    `https://api.caiyunapp.com/${params.apiVersion}/${params.token}/${longitude},${latitude}/weather`,
  );
  ...
  const dailyTemperature = Array.isArray(daily.temperature)
    ? daily.temperature.slice(0, params.days)
    : [];
  const dailySkycon = Array.isArray(daily.skycon)
    ? daily.skycon.slice(0, params.days)
    : [];
  const dailyPrecipitation = Array.isArray(daily.precipitation)
    ? daily.precipitation.slice(0, params.days)
    : [];
  ...
  return {
    ...
    forecast: dailyTemperature.map((item, index) => {
      ...
      const sky = isRecord(dailySkycon[index])
        ? (dailySkycon[index] as Record<string, unknown>)
        : {};
      const precipitation = isRecord(dailyPrecipitation[index])
        ? (dailyPrecipitation[index] as Record<string, unknown>)
        : {};
      return {
...
export function createWeatherTool(options?: WeatherToolOptions): Tool {
  ...
  const parameters = Type.Object({
    location: Type.Optional(
      Type.String({ description: "City or place name, e.g. 香港, 深圳, Beijing" }),
    ),
    latitude: Type.Optional(
      Type.Number({
        description: "Latitude, recommended for Caiyun queries",
        minimum: -90,
        maximum: 90,
      }),
    ),
    longitude: Type.Optional(
      Type.Number({
        description: "Longitude, recommended for Caiyun queries",
        minimum: -180,
        maximum: 180,
      }),
    ),
    ...
    provider: Type.Optional(
      Type.Union(
        [Type.Literal("wttr"), Type.Literal("caiyun")],
  ...
  return {
    ...
    label: "Weather",
    description:
      "Query current weather and short forecast. Supports wttr (free) and Caiyun (requires caiyunApiKey option). For Caiyun, provide latitude/longitude or a known city name.",
    parameters,
    execute: async (_toolCallId, args, _signal, _onUpdate, _ctx) => {
      try {
        ...
        const location =
          params.location ?? config?.defaultLocation ?? "";
        const latitude = params.latitude;
        const longitude = params.longitude;
        const days = Math.max(
          1,
          Math.min(7, params.days ?? 3),
        );
        const timeoutMs =
        ...
        if (
          ...
          return errorResult(
            "location or latitude/longitude is required",
          );
        }
        ...
        if (provider === "caiyun") {
          const token = config?.caiyunApiKey?.trim();
          if (!token) {
            return errorResult(
              "Caiyun weather requires caiyunApiKey option",
            );
          }
          ...
          if (!coordinates) {
            return errorResult(
              "Caiyun weather requires latitude/longitude or a known city name",
            );
          }
          result = await queryCaiyun({
            ...
            coordinates,
            token,
            apiVersion: config?.caiyunApiVersion ?? "v2.6",
            timeoutMs,
            days,
          });
        } else {
          result = await queryWttr({
            ...
            baseUrl: config?.wttrBaseUrl ?? "https://wttr.in",
            timeoutMs,
          });
        }
