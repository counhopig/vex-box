...
import { getSession, getFinishedSession, listRunningSessions, listFinishedSessions, killSession, deleteSession, drainSession, sliceLogLines, tail, formatDuration, GLOBAL_OWNER_KEY } from "./process-registry.js";

type ProcessAction = "list" | "poll" | "log" | "write" | "kill" | "clear";

export function createProcessTool(ownerKey: string = GLOBAL_OWNER_KEY): AgentTool {
  return {
    name: "process",
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        case "list": return handleList(ownerKey);
        case "poll": return handlePoll(ownerKey, sessionId);
        case "log": return handleLog(ownerKey, sessionId, params);
        case "write": return handleWrite(ownerKey, sessionId, params);
        case "kill": return handleKill(ownerKey, sessionId);
        case "clear": return handleClear(ownerKey, sessionId);
...
function handleList(ownerKey: string) {
  const running = listRunningSessions(ownerKey);
  const finished = listFinishedSessions(ownerKey);
...
function handlePoll(ownerKey: string, sessionId?: string) {
  if (!sessionId) return jsonResult({ status: "error", error: "session_id is required" }, true);
  let session = getSession(sessionId, ownerKey);
  let isFinished = false;
  if (!session) { session = getFinishedSession(sessionId, ownerKey); isFinished = true; }
...
function handleLog(ownerKey: string, sessionId?: string, params?: Record<string, unknown>) {
  if (!sessionId) return jsonResult({ status: "error", error: "session_id is required" }, true);
  ...
  const session = getSession(sessionId, ownerKey) ?? getFinishedSession(sessionId, ownerKey);
  if (!session) return jsonResult({ status: "error", error: `Session not found: ${sessionId}` }, true);
...
function handleWrite(ownerKey: string, sessionId?: string, params?: Record<string, unknown>) {
  if (!sessionId) return jsonResult({ status: "error", error: "session_id is required" }, true);
  ...
  const session = getSession(sessionId, ownerKey);
...
function handleKill(ownerKey: string, sessionId?: string) {
  if (!sessionId) return jsonResult({ status: "error", error: "session_id is required" }, true);
  const session = getSession(sessionId, ownerKey);
  if (!session) return jsonResult({ status: "error", error: `Session not found or already finished: ${sessionId}` }, true);
  killSession(session);
...
function handleClear(ownerKey: string, sessionId?: string) {
  if (!sessionId) return jsonResult({ status: "error", error: "session_id is required" }, true);
  const deleted = deleteSession(sessionId, ownerKey);
